...
Comprehensive multimodal fusion comparison:
  - Concat + LogReg (grid search)
  - Concat + SVM-RBF (grid search)
  - FCN (learnable, regular CE)
  - FCN Balanced (learnable, class-weighted CE)

All 7 modality pairs, k=5 and k=10, PCA fitted inside each fold.

Usage:
    python fusion_all_pairs.py --thresholds 10
    python fusion_all_pairs.py --thresholds 10 8 5
    python fusion_all_pairs.py --thresholds 10 --pairs VGG19+HuBERT ViT+VGG19
"""
...
def run_concat_gridsearch(X_a, X_b, y, n_classes, pca_dim, n_folds, classifier="lr"):
    """
    Concat fusion with sklearn GridSearchCV.
    PCA per modality is handled inside a custom approach:
      - We PCA each modality, concat, then run classifier grid search.
      - To keep PCA inside each fold, we use a Pipeline on the concatenated space
        with a single PCA (since both modalities are already reduced).

    Actually, for proper per-modality PCA inside fold, we do manual k-fold:
      For each fold:
    ...
    for train_idx, val_idx in cv.split(X_a, y):
        ...
        X_a_tr_pca = norm.fit_transform(X_a_tr_pca)
        X_a_val_pca = norm.transform(X_a_val_pca)
        X_b_tr_pca = norm.fit_transform(X_b_tr_pca)
        X_b_val_pca = norm.transform(X_b_val_pca)

        # Concat
        X_tr_cat = np.concatenate([X_a_tr_pca, X_b_tr_pca], axis=1)
        X_val_cat = np.concatenate([X_a_val_pca, X_b_val_pca], axis=1)

        # Fit classifier with inner CV for hyperparam selection
        if classifier == "lr":
            # Simple: fit best LR from our known good params
...
def main():
    ...
    parser.add_argument("--methods", nargs="+", type=str,
                        default=["concat_lr", "concat_svm", "fcn", "fcn_balanced"],
                        help="Methods to run")
    ...
    print("\n" + "=" * 110)
    print("  COMPREHENSIVE FUSION COMPARISON — All Pairs × All Methods × K=5,10")
    print("  PCA fitted inside each fold (NO data leakage)")
    print("=" * 110)
    print(f"\n  Pairs: {pairs_to_run}")
    print(f"  Thresholds: {thresholds}")
    print(f"  K-Folds: {K_FOLDS}")
    print(f"  PCA dim per modality: {pca_dim}")
    print(f"  Methods: {args.methods}")
    print(f"  Concat+LR: C=1000, balanced, lbfgs")
    print(f"  Concat+SVM: C=100, RBF, gamma=scale, balanced")
    print(f"  FCN: {pca_dim*2}→{HIDDEN_DIM}→ReLU→Drop({DROPOUT})→n_classes, epochs={EPOCHS}, patience={PATIENCE}")
    print()
            ...
            for k in K_FOLDS:
                ...
                if "concat_lr" in args.methods:
                    ...
                    if mean_f1 is not None:
                        ...
                        all_results.append({
                            "pair": pair_name, "method": "Concat+LR", "k": k,
                ...
                if "concat_svm" in args.methods:
                    ...
                    if mean_f1 is not None:
                        ...
                        print(f"{mean_f1:.4f} ±{std_f1:.4f} ({elapsed:.0f}s) [{folds_str}]")
                        all_results.append({
                            "pair": pair_name, "method": "Concat+SVM", "k": k,
                            "threshold": threshold, "n_classes": n_classes,
                            "n_samples": len(y_f), "mean_f1": mean_f1,
                            "std_f1": std_f1, "fold_scores": folds,
                        })
                ...
                if "fcn_balanced" in args.methods:
                    ...
                    if mean_f1 is not None:
                        ...
                        print(f"{mean_f1:.4f} ±{std_f1:.4f} ({elapsed:.0f}s) [{folds_str}]")
                        all_results.append({
                            "pair": pair_name, "method": "FCN_Balanced", "k": k,
                            "threshold": threshold, "n_classes": n_classes,
                            "n_samples": len(y_f), "mean_f1": mean_f1,
                            "std_f1": std_f1, "fold_scores": folds,
                        })
    ...
    print(f"{'=' * 110}")
    print(f"{'Pair':<16} | {'Method':<13} | {'K':<3} | {'Thresh':<6} | {'Classes':<7} | "
          f"{'Macro-F1':<9} | {'Std':<6} | {'Best Fold':<9}")
    print(f"{'─' * 110}")

    for r in sorted(all_results, key=lambda x: (-x["threshold"], -x["mean_f1"])):
        best_fold = max(r["fold_scores"])
        print(
            f"{r['pair']:<16} | {r['method']:<13} | {r['k']:<3} | {r['threshold']:<6} | "
    ...
    # =====================================================
    # COMPARISON TABLES (per K value)
    # =====================================================

    for k in K_FOLDS:
        print(f"\n  === K={k} SUMMARY (sorted by Macro-F1) ===")
        print(f"  {'Pair':<16} | {'Method':<13} | {'Macro-F1':<9} | {'Std':<6}")
        print(f"  {'─' * 60}")
        k_results = [r for r in all_results if r["k"] == k]
        for r in sorted(k_results, key=lambda x: -x["mean_f1"]):
            print(f"  {r['pair']:<16} | {r['method']:<13} | {r['mean_f1']:<9.4f} | {r['std_f1']:<6.4f}")

    # =====================================================
    # BEST PER PAIR
    # =====================================================

    print(f"\n  === BEST METHOD PER PAIR (across all K) ===")
    print(f"  {'─' * 80}")
    for pair_name in pairs_to_run:
        subset = [r for r in all_results if r["pair"] == pair_name]
        if subset:
            best = max(subset, key=lambda x: x["mean_f1"])
            print(f"  {pair_name:<16}: {best['method']} (K={best['k']}) → "
                  f"{best['mean_f1']:.4f} ±{best['std_f1']:.4f}")

    # =====================================================
    # COMPARISON WITH SINGLE-MODALITY BASELINES
    # =====================================================

    print(f"\n  === vs SINGLE-MODALITY BASELINES (LR Extended) ===")
    print(f"  {'─' * 80}")
    baselines = {
        10: {"HuBERT": 0.6607, "ViT": 0.6159, "VGG19": 0.5993, "WavLM": 0.6043, "Wav2Vec2": 0.5370},
        8: {"HuBERT": 0.6438, "ViT": 0.6140, "VGG19": 0.5993, "WavLM": 0.5843, "Wav2Vec2": 0.5288},
        5: {"HuBERT": 0.5718, "ViT": 0.5241, "VGG19": 0.5256, "WavLM": 0.5168, "Wav2Vec2": 0.4643},
    }
    ...
    for threshold in thresholds:
        ...
        for pair_name in pairs_to_run:
            ...
            print(f"    {pair_name:<16}: {best['mean_f1']:.4f} vs best_single={best_single:.4f} "
                  f"({arrow}{abs(diff):.4f}) [{best['method']}, K={best['k']}]")
