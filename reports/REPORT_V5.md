...
4. **Audio model ranking (base-size models):** HuBERT >> WavLM > Wav2Vec2 *(Note: superseded at large scale — see Section 17/18. WavLM-Large masked_mean_max = 0.7444 vs HuBERT-base = 0.6870)*
...
|------|--------|--------------|-------|
| 1 | **WavLM-Large masked_mean_max (unimodal)** | **0.7444** | Single-modality (Section 18), seed=42 only, not multi-seed validated |
| 2 | **Three-encoder FCN_Balanced** | **0.7264 ± 0.0075** | Multi-seed validated three-encoder fusion |
| 3 | **Two-encoder FCN_Balanced** | **0.7231 ± 0.0073** | Multi-seed validated DINOv2+HuBERT |
| 4 | Seed=42 optimized FCN_Balanced | 0.7361 | High single-seed result, not expected performance |
| 5 | Seed=42 three-encoder FCN_Balanced | 0.7414 | Highest observed single-seed fusion result |
| 6 | Late fusion weighted average | 0.7020 | Decision-level fusion underperforms feature-level |
| 7 | Hyperbolic cross-attention v3 | 0.6990 | Higher complexity but lower performance |
...
HuBERT-base consistently outperforms WavLM-base by ~2.7pp as a fusion partner. However, this finding applies only to base-size models. The new large PTM benchmark (Section 17) and the sequence pooling benchmark (Section 18) show that **WavLM-Large with masked_mean_max pooling (0.7444) far exceeds HuBERT-base (0.6870)** as a unimodal encoder, and WavLM-Large is likely the superior fusion partner at large scale.
...
- **Best single-modality (large models, Section 17, mean-only pooling):** WavLM-Large masked SVM-RBF = **0.7060**
- **Best single-modality (Section 18, full pooling sweep):** WavLM-Large masked_mean_max SVM_bal_pca128 = **0.7444**
...
The best defensible multi-seed validated configuration remains **DINOv2+HuBERT+ConvNeXt with class-balanced FCN fusion**, achieving **0.7264 ± 0.0075 Macro-F1** under five-seed validation at threshold ≥10. However, the new sequence-embedding pooling benchmark (Section 18) shows that **WavLM-Large with masked_mean_max pooling achieves 0.7444 Macro-F1** as a single-modality encoder — exceeding the multi-seed validated fusion result by +0.0180 (seed=42 only, not yet multi-seed validated). This suggests improved temporal pooling of waveform SSL features may be more impactful than adding visual modalities, though multi-seed validation of the 0.7444 result and fusion with WavLM-Large masked_mean_max remain future work.
...
| `eval_pooled_from_sequence_embeddings.py` | Section 18: Pool saved [N,T,D] sequences with multiple strategies, evaluate classifiers |
| `eval_pooled_neural_heads.py` | Section 18.10: Neural heads (PooledMLP, PooledFOCA_CNN_Adaptive) on pooled sequences |
| `eval_sequence_heads.py` | Section 18.11: Full-sequence CNN/attention heads on raw [N,T,D] |
| `results/sequence_unimodal_benchmark_pooling.md` | Full pooled-from-sequence results (all thresholds, all models) |
...
**Audio:** WavLM-Large (masked_mean_max — see Section 18 for pooling benchmark), AST, BEATs (unmasked)  
**Reference:** HuBERT-base (historical fusion anchor)

> *(Section 18 supersedes this section for unimodal benchmarking: WavLM-Large masked_mean_max = 0.7444 is now the strongest single-modality result, exceeding even multi-seed validated fusion.)*
...
## 18. Sequence Embedding Benchmark: Pooled from Saved [N, T, D] Representations

### 18.1 Motivation

Section 17 evaluated frozen PTM embeddings using only their **pre-extracted pooled vectors** (one vector per sample, pooled at extraction time). However, PTMs produce rich intermediate token/frame-level sequences — e.g., WavLM-Large outputs `[N, T=1499, D=1024]` for audio, and DINOv2-Large outputs `[N, P=1369, D=1024]` for image patch tokens.

To unlock stronger representations, full sequence embeddings were extracted and saved locally:

- **Audio models:** `[N, T, D]` with variable-length `lengths.pt` for masking (WavLM-Large, HuBERT-base, BEATs, AST)
- **Image models:** `[N, P, D]` fixed-length patch embeddings (DINOv2-Large, DINOv2-Large-Reg, ConvNeXt-B)

This section evaluates **post-hoc pooling strategies** over saved sequences — comparing multiple ways to compress `[N, T, D]` → `[N, D]` or `[N, 2D]` before feeding to classifiers. This decouples the pooling decision from extraction, enabling systematic pooling exploration without re-running costly models.

### 18.2 Models & Saved Sequence Shapes

| Model | Modality | Sequence Shape | Dim | Has Lengths |
|-------|----------|---------------|-----|-------------|
| WavLM-Large | Audio | [3095, 1499, 1024] | 1024 | Yes (min=6, max=1499) |
| HuBERT-base | Audio | [3095, 1499, 768] | 768 | Yes (min=6, max=1499) |
| BEATs | Audio | [3095, 1496, 768] | 768 | Yes (all=1496) |
| AST | Audio | [3095, 1212, 768] | 768 | Yes (all=1212) |
| DINOv2-Large | Image | [3094, 1369, 1024] | 1024 | No |
| DINOv2-Large-Reg | Image | [3094, 1369, 1024] | 1024 | No |
| ConvNeXt-B | Image | [3094, 49, 1024] | 1024 | No |

### 18.3 Pooling Strategies

| Pooling | Description | Output Dim | Applicable Models |
|---------|-------------|------------|-------------------|
| masked_mean | Mean over valid frames only (respects `lengths.pt`) | D | Audio only |
| masked_max | Max over valid frames only | D | Audio only |
| masked_mean_max | Concat of masked_mean and masked_max | 2D | Audio only |
| unmasked_mean | Mean over all frames (including padding) | D | Audio only |
| mean | Mean over all patch tokens | D | Image only |
| max | Max over all patch tokens | D | Image only |
| mean_max | Concat of mean and max | 2D | Image only |

### 18.4 Evaluation Protocol

- **CV:** 5-Fold StratifiedKFold (seed=42)
- **PCA:** Fitted inside each fold (no leakage)
- **Metric:** Macro-F1
- **Classifiers:** LR_bal_C1k, LR_C1k, LR_bal_C100, LR_C100, SVM_bal, SVM, MLP
- **PCA dims:** LR: [128, 256, 384, 512]; SVM: [128, 256]; MLP: [128, 256, 512]
- **Normalization:** LR → L2 Normalizer; SVM/MLP → StandardScaler
- **Total configs per threshold:** 575 (7 models × 4–7 pooling variants × 7 classifiers × 2–4 PCA dims)
- **Script:** `eval_pooled_from_sequence_embeddings.py`
- **Status:** Complete for all three thresholds (575 configs each)

### 18.5 Results — Threshold ≥10 (1,947 samples, 89 classes)

#### Top 10 Overall

| Rank | Model | Pooling | Classifier | PCA | Macro-F1 | Std |
|------|-------|---------|-----------|-----|----------|-----|
| 1 | **WavLM-Large** | **masked_mean_max** | **SVM_bal** | 128 | **0.7444** | 0.0151 |
| 2 | WavLM-Large | masked_mean_max | SVM | 128 | 0.7442 | 0.0146 |
| 3 | WavLM-Large | masked_max | SVM_bal | 128 | 0.7413 | 0.0117 |
| 4 | WavLM-Large | masked_max | SVM | 128 | 0.7391 | 0.0111 |
| 5 | WavLM-Large | masked_max | LR_bal_C1k | 512 | 0.7372 | 0.0157 |
| 6 | WavLM-Large | masked_max | LR_bal_C100 | 512 | 0.7355 | 0.0136 |
| 7 | WavLM-Large | masked_max | LR_bal_C100 | 384 | 0.7329 | 0.0146 |
| 8 | WavLM-Large | masked_max | LR_C1k | 512 | 0.7321 | 0.0138 |
| 9 | WavLM-Large | masked_mean_max | LR_C1k | 384 | 0.7313 | 0.0109 |
| 10 | WavLM-Large | masked_mean_max | LR_C1k | 512 | 0.7312 | 0.0129 |

#### Best per Model

| Model | Pooling | Classifier | PCA | Macro-F1 | Prev. Baseline (§17) | Δ |
|-------|---------|-----------|-----|----------|----------------------|------|
| **WavLM-Large** | masked_mean_max | SVM_bal | 128 | **0.7444** | 0.7060 | **+0.0384** |
| AST | masked_mean | SVM_bal | 128 | 0.7006 | 0.6886 | +0.0120 |
| BEATs | masked_mean_max | SVM_bal | 128 | 0.6993 | 0.6823 | +0.0170 |
| HuBERT-base | masked_mean | SVM_bal | 128 | 0.6916 | 0.6870 | +0.0046 |
| DINOv2-Large | mean | SVM_bal | 128 | 0.6817 | 0.6884 | −0.0067 |
| DINOv2-Large-Reg | mean_max | SVM_bal | 128 | 0.6806 | 0.6873 | −0.0067 |
| ConvNeXt-B | mean | SVM_bal | 128 | 0.6730 | 0.6578 | +0.0152 |

#### Best per Pooling Strategy

| Pooling | Best Model | Macro-F1 |
|---------|-----------|----------|
| masked_mean_max | WavLM-Large | **0.7444** |
| masked_max | WavLM-Large | 0.7413 |
| masked_mean | WavLM-Large | 0.7260 |
| unmasked_mean | WavLM-Large | 0.7060 |
| mean | DINOv2-Large | 0.6817 |
| mean_max | DINOv2-Large-Reg | 0.6806 |
| max | DINOv2-Large | 0.6705 |

#### WavLM-Large Breakdown by Pooling

| Pooling | Best Config | PCA | Macro-F1 | vs Baseline |
|---------|-------------|-----|----------|-------------|
| masked_mean_max | SVM_bal | 128 | **0.7444** | +0.0384 |
| masked_max | SVM_bal | 128 | 0.7413 | +0.0353 |
| masked_mean | SVM_bal | 128 | 0.7260 | +0.0200 |
| unmasked_mean | SVM_bal | 128 | 0.7060 | ±0.0000 |

### 18.6 Cross-Threshold Summary (Best per Model)
...
1. **WavLM-Large + masked_mean_max = 0.7444** is the new single-modality state of the art at t≥10. This is +0.0384 above the previous best WavLM-Large masked_mean (0.7060) from Section 17, and +0.0574 above HuBERT-base SVM (0.6870) from Section 4.

2. **masked_mean_max is the best pooling strategy** — concatenating masked-mean and masked-max (`[N, 2D]`) consistently outperforms either alone. The masked-max component captures feature-wise peaks while masked-mean captures average activation patterns; their combination provides a richer representation.

3. **Masking provides significant gains for variable-length audio** — WavLM-Large: masked_mean (0.7260) vs unmasked_mean (0.7060) = +0.0200. Padding frames introduce noise that dilutes discriminative information.

4. **Image models do NOT benefit** from pooling exploration — DINOv2-Large/Reg show slight regressions (−0.0067) vs their Section 17 baselines. Their existing CLS-token or mean-pool embeddings already capture most spatial information; post-hoc re-pooling from patch tokens does not add value.

5. **Audio models benefit substantially** — all four audio models improve: WavLM (+0.0384), BEATs (+0.0170), AST (+0.0120), HuBERT (+0.0046). This confirms that the choice of temporal pooling is critical for waveform SSL models.

6. **SVM_bal with PCA=128 remains the dominant classifier configuration** — wins 5 of 7 models at t≥10, consistent with findings from Sections 4 and 17.

### 18.8 Updated Unimodal Rankings

**Previous best (Section 17, masked mean only):** WavLM-Large = 0.7060  
**New best (this section, masked_mean_max):** WavLM-Large = **0.7444**

This is the strongest unimodal result in the entire report. It also exceeds the validated 2-encoder fusion result from Section 11.6 (DINOv2+HuBERT FCN_Balanced = 0.7231 ± 0.0073), though that comparison is between a single-seed unimodal result and a multi-seed validated fusion result.

### 18.9 Implications for Fusion

WavLM-Large masked_mean_max (0.7444) now **exceeds the multi-seed validated DINOv2+HuBERT fusion** (0.7231). This suggests:

- The previous fusion pipeline used base-size HuBERT (768d, unmasked mean) and base-size DINOv2. A revised fusion pipeline using WavLM-Large masked_mean_max (2048d) as the audio backbone is a priority for future experiments.
- The gain from improved temporal pooling (+0.0384) far exceeds the gain from adding a second modality (+0.0353 from Sections 4→6).

### 18.10 Ongoing Work: Pooled Neural Heads (In Progress)

A neural head benchmark (`eval_pooled_neural_heads.py`) is currently running on the same pooled representations. This evaluates PooledMLP and PooledFOCA_CNN_Adaptive heads with a grid over:

- **Heads:** PooledMLP, PooledFOCA_CNN_Adaptive
- **Input normalization:** standard (StandardScaler), none, layernorm
- **Optimizers:** AdamW, Adam
- **Learning rates:** [3e-4, 1e-4, 3e-5, 1e-5]
- **Dropout:** [0.3, 0.5]
- **Weight decay:** [0.0, 1e-5, 1e-4, 1e-3]
- **Total configs:** 3,072 (8 model/pooling pairs × 2 heads × 2 optimizers × 4 LRs × 2 dropouts × 4 WDs × 3 input norms)
- **Total fold trainings:** 15,360

**Status:** 40/3,072 configs completed (1.3%). Results pending — do not interpret preliminary numbers as final.

### 18.11 Ongoing Work: Full-Sequence CNN/Attention Heads (Incomplete)

A separate experiment evaluates heads that operate directly on the full `[N, T, D]` sequences (without pre-pooling), using architectures like sliding-window Conv1D, attention-pooled classifiers, and MeanMaxMLP. This was partially run on macOS and a larger grid has been submitted to a cluster (OMNI). Results are incomplete and will be reported after all jobs finish.

**Local Mac status:** 155/unknown configs done across convnext_base, ast, beats, hubert_base (stopped early).  
**Cluster (OMNI):** Submitted but not yet complete.  
**Conclusion:** Deferred until cluster results return.

> Full pooling benchmark details available in [`results/sequence_unimodal_benchmark_pooling.md`](results/sequence_unimodal_benchmark_pooling.md).
